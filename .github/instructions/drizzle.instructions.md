 ...
 * @param db - Injectable database client; use an in-memory client in tests.
 * @returns The matching game ids, ordered by title.
 */
...
- Add a TSDoc/JSDoc comment to every exported function in `db/` and `src/lib/`, including exported function-valued variables. State the function's purpose and describe each parameter and the return value with `@param` and `@returns` tags.
- For helpers that accept `db`, document that parameter as the injected Drizzle client and explain that injection allows callers and tests to supply the appropriate database. Keep `db` as the first parameter.
- Keep comments focused on purpose and non-obvious reasoning. Do not narrate individual statements; update or remove comments whenever the related behavior changes.
